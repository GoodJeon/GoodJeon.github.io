...
layout: posts
comments: true
title: "[BOJ/백준-Python]1620번 - 나는야 포켓몬 마스터 이다솜"
...
시간초과에 걸리지 않기 위해 딕셔너리를 사용해야하는 문제다.
...
포켓몬의정보(번호, 이름)이 들어갈 딕셔너리형 자료를 만들어주자.

```python
pocketmon_dict = {}
```

`N`번만큼 문자열을 받아주는데, 이 때 예제처럼 포켓몬 확인 시 숫자로 문제를 받을 수도 있고, 이름으로 받을 수도 있다. 따라서, 범위보다 1큰 수를 키값에 넣고 포켓몬 이름을 밸류에 넣거나, 그 반대의 경우도 저장해주자. 그리고 `sys.stdin.readline()`을 이용할 땐, 맨 뒤에는 개행문자가 나오니 슬라이싱을 통해 날려주던가 하자!

만약 Thunder라는 포켓몬이 처음으로 들어가게되면 자료형에는 이런식으로 저장된다.
...
for i in range(N):
  ...
  pocketmon_dict[i+1] = a
  pocketmon_dict[a] = i + 1
```

이제 주어진 문제에 대한 답을 해야한다.

문제는 숫자 혹은 문자열로 주어진다.

`M`번 만큼 반복해서 문제를 받아 준 뒤에, 숫자면 int형 변환을 이용해 키값에 넣어주고, 아니라면 그냥 넣어주자.

`isdigit()`으로 숫자인지를 판별할 수 있다.

```python
for j in range(M):
  a = sys.stdin.readline()[:-1]
  if a.isdigit():
    print(pocketmon_dict[int(a)])
  else:
    print(pocketmon_dict[a])
```
